...
export const apiSpaceScanGet = (path, controller) => {
  return spaceScanApi
    .get(path, controller)
    .then((response) => {
      if (response) {
...
export const apiSpaceScanMultiGet = (paths, controller) => {
  const promises = paths.map((path) => apiSpaceScanGet(path, controller));
  return Promise.all(promises);
};
